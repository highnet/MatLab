...
% variables
%
%  Given two points in 3D Space (position) represented by the vectors P1 = [x1,y1,z1] 
% and P2 = [x2,y2,z2], the the angle of the line from P1 to P2 with respect
%to the x-axis defined by theta = arccos(dot(P1,P2) / sqrt(magnitude(P1) *
% magnitude(P2)))
% dot(a,b) = x1x2+y1y2+z1z2 and magnitude(c)=(x1^2 + y1^2 + z1^1)

alpha = acosd(dot(P1,P2) / sqrt(sum(P1.^2)*sum(P2.^2)));
beta = acosd(dot(P1,P3) / sqrt(sum(P1.^2)*sum(P3.^2)));
gamma = acosd(dot(P2,P3) / sqrt(sum(P2.^2)*sum(P3.^2)));
